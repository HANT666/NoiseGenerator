...
namespace NoiseGenerator
{
    class Perlin2D
    {
        ...
        int size;
        public Perlin2D(int size, int seed = 0)
        {
            this.size = size;
        ...
        public float[,] GetData(int octaves, float persistance)
        {
            float[,] dataFloat = new float[size, size];

            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size; x++)
                {
                    dataFloat[x, y] = (float)Math.Floor((Noise((x / (float)(size - 1)), (y / (float)(size - 1)), octaves, persistance) + 1) * 127.5f);
                }
            }
            return dataFloat;
        }
